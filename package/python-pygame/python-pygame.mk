...
PYTHON_PYGAME_SDL_CFLAGS = $(STAGING_DIR)/usr/bin/sdl-config --cflags
PYTHON_PYGAME_SDL_LIBS = $(STAGING_DIR)/usr/bin/sdl-config --libs
...
define PYTHON_PYGAME_CONFIGURE_CMDS
	...
	$(SED) "s~^SDL = ~SDL = `$(PYTHON_PYGAME_SDL_CFLAGS)``$(PYTHON_PYGAME_SDL_LIBS)` \n#~" $(@D)/Setup
